...
object SilverTransformer {
  ...
  def transform(raw: DataFrame)(implicit spark: SparkSession): DataFrame = {
    ...
    // 2) Remove empty content and add content hash
    val withHash = cleaned
      .filter(length(col("content")) > 0)
      .withColumn("content_hash", sha2(col("content"), 256))

    // 3) Deduplicate by content_hash → keep latest ingested_at
    ...
    val enriched = deduped
      .withColumn("doc_id", expr("uuid()")) // unique ID
      .withColumn("silver_timestamp", current_timestamp())
      .withColumn("source_system", lit("LakeRAG-Scala-ETL")) // constant (you can change)
      .withColumn("language", lit("unknown")) // optional — real detection can be added later

    // Final canonical silver schema (aligned with SilverRecord)
    enriched.select(
      col("doc_id"),
      col("file_path"),
      col("content"),
